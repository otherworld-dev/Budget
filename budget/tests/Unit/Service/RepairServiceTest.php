...
class RepairServiceTest extends TestCase {
    ...
    public function testFindTransferCreditsWithCategory(): void {
        $account = $this->makeAccount(['id' => 1, 'name' => 'Savings']);
        $this->withAccounts([$account]);
        $this->withActiveBills([]);

        $creditTx = $this->makeTransaction([
            'id' => 30,
            'date' => '2026-05-01',
            'amount' => '200.00',
            'type' => 'credit',
            'categoryId' => 5, // should be null for credit-side transfer
            'vendor' => 'Transfer',
            'description' => '',
            'accountId' => 1,
        ]);
        ...
        $this->assertCount(1, $result['transferCreditCategories']);
        $this->assertEquals(30, $result['transferCreditCategories'][0]['transactionId']);
        $this->assertEquals(5, $result['transferCreditCategories'][0]['categoryId']);
    }
    ...
    // ---------------------------------------------------------------
    // 18. repair transferCreditCategories - clears category
    // ---------------------------------------------------------------

    public function testRepairTransferCreditCategoriesClearsCategory(): void {
        $account = $this->makeAccount(['id' => 1, 'name' => 'Savings']);
        $this->withAccounts([$account]);
        $this->withActiveBills([]);
        ...
        $this->assertEquals(1, $result['transferCreditCategories']['fixed']);
        $this->assertEquals(1, $result['transferCreditCategories']['found']);
    }
