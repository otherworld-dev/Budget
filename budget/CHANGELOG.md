...
### Added
- **A recurring transfer can now pay a credit card's statement balance, resolved fresh at every payment.** Card payments have always been representable as recurring transfers, but only at a fixed amount — useless for a statement that is different every month, which left people faking it with duplicate bills or typing the figure in by hand each time. A transfer whose destination is a credit card (or line of credit) now offers an **Amount type** choice: *Fixed amount* as before, or *Statement balance*, which pays whatever is owed on the card as of the due date — everything charged since the previous due date, plus any remainder an underpayment carried forward. The figure is worked out from the card's own ledger at the moment you mark the payment made (or auto-pay makes it), so it is never stale; the amount shown on the transfer between payments is the last statement resolved, and the pre-booked next occurrence carries it as the forecast estimate. Reminders, the bills calendar and auto-pay all work unchanged, because the payment *is* an ordinary recurring transfer. The card's own account page joins in: a **Set up payment** button appears on a card with no payment transfer and opens the form pre-filled, and once one exists the page shows when the next payment is due ([#347](https://github.com/otherworld-dev/Budget/issues/347))
