...
- **The Bills Calendar says what is left in an account after each month's bills.** With an account chosen in the calendar's Account filter, a **Balance after bills** row under the monthly totals takes the bills still to be paid in each month off the account's balance today and adds any transfers still due to arrive into it, so the current month shows what will be left once the rest of its bills go out, and the months ahead show whether today's balance would cover them. A month with nothing left to pay is blank, a shortfall is shown in red, and hovering the row gives the balance the figures start from. Bills paying from other accounts are left out, and a bill the calendar has crossed out is already settled. The CSV and PDF exports carry the same row. With **All Accounts** selected there is no single balance to start from, so the row is not shown. Suggested by @TerjeTM ([#393](https://github.com/otherworld-dev/Budget/issues/393))
