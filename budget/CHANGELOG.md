...
- **The Transactions page works on a phone.** At phone width the table only had room for three columns, so it dropped the description and cut the amount off at the edge of the screen. Each transaction is now a two-line card, with the description and amount on the first line and the date, category and account under them. On any screen size, Reconcile, Find Duplicates, Match All and Export have moved into a **⋯** menu beside **Add Transaction** so the buttons fit on one line next to the title, and the **⋮** menu on each row is a list with names instead of a strip of unlabelled icons. **Delete** is in that menu now rather than sitting on every row.
